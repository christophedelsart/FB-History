/*1314748121,169905780,JIT Construction: v432523,fr_FR*/
...
FB.provide('UA', {
    ...
    iPad: function () {
        return FB.UA._populate() || this._iPad;
    },
    nativeApp: function () {
        return false;
    },
...
FB.provide('UIServer', {
    ...
    _handleOrientationChange: function (b) {
        ...
        } else a.style.width = FB.UIServer.getDefaultSize().width + 'px';
    },
...
FB.subclass('XFBML.SocialBar', 'XFBML.IframeWidget', null, {
    ...
    setupAndValidate: function () {
        ...
        var a = FB.bind(function () {
            ...
            window.setTimeout(FB.bind(this.arbiterInform, this, 'platform/plugins/social_bar/action'), this._attr.read_time * 1000);
            return true;
        }, this);
        ...
        if (this._attr.trigger == "manual") {
            var c = FB.bind(function (h) {
                if (h == this._attr.href) {
                    FB.Event.unsubscribe('xfbml.socialbar.read', c);
                    this.arbiterInform('platform/plugins/social_bar/trigger');
                }
                return true;
            }, this);
            FB.Event.subscribe('xfbml.socialbar.read', c);
        } else {
...
FB.XFBML.SocialBar.markRead = function (a) {
    FB.Event.fire('xfbml.socialbar.read', a || window.location.href);
};
void(0);
