/*1312935162,169593460,JIT Construction: v419814,fr_FR*/
...
FB.provide('Dialog', {
    ...
    _createMobileLoader: function () {
        var a = FB.UA.nativeApp() ? '' : ('<table>' + '  <tbody>' + '    <tr>' + '      <td class="header_left">' + '        <label class="touchable_button">' + '          <input type="submit" value="' + FB.Intl._tx("Annuler") + '"' + '            id="fb_dialog_loader_close"/>' + '        </label>' + '      </td>' + '      <td class="header_center">' + '        <div>' + FB.Intl._tx("Chargement...") + '</div>' + '      </td>' + '      <td class="header_right">' + '      </td>' + '    </tr>' + '  </tbody>' + '</table>');
        return FB.Dialog.create({
            classes: 'loading',
    ...
    scrollToTop: function () {
        if (FB.UA.mobile() && FB.Dialog._active) {
            FB.Dialog._lastYOffset = window.pageYOffset;
            window.scrollTo(0, 0);
        }
    },
    restorePosition: function () {
        if (FB.UA.mobile()) window.scrollTo(0, FB.Dialog._lastYOffset);
    },
    showLoader: function (a, c) {
        if (!FB.Dialog._loaderEl) FB.Dialog._loaderEl = FB.Dialog._findRoot(FB.UA.mobile() ? FB.Dialog._createMobileLoader() : FB.Dialog._createWWWLoader(c));
        if (!a) a = function () {};
        var b = FB.$('fb_dialog_loader_close');
        FB.Dom.removeCss(b, 'fb_hidden');
        b.onclick = function () {
            FB.Dialog._hideLoader();
            FB.Dialog.restorePosition();
            a();
        };
        FB.Dialog._makeActive(FB.Dialog._loaderEl);
        FB.Dialog.scrollToTop();
    },
...
FB.provide('UIServer', {
    ...
    _handleOrientationChange: function (b) {
        var a = FB.UIServer._loadedNodes[b];
        if (!a) return;
        a.style.width = FB.UIServer.getDefaultSize().width + 'px';
        FB.Dialog.scrollToTop();
    },
    _addOrientationHandler: function (b) {
        if (!FB.UA.mobile()) return;
        FB.UIServer._handleOrientationChange(b);
        var a = "onorientationchange" in window ? 'orientationchange' : 'resize';
        window.addEventListener(a, FB.bind(FB.UIServer._handleOrientationChange, null, b));
    },
    _handleResizeMessage: function (b, a) {
        ...
        FB.UIServer._addOrientationHandler(b);
...
FB.provide('', {
    ...
    getAccessToken: function () {
        ...
        return (FB._session && FB._session.access_token) || null;
    },
    getUserID: function () {
        if (FB._oauth) {
            return FB._userID;
        } else
        return (FB._session && FB._session.uid) || 0;
    },
    login: function (a, b) {
        if (FB._oauth) {
...
FB.provide('Auth', {
    ...
    setAuthResponse: function (a, g) {
        ...
        FB._userStatus = g;
        if (e || c) FB.Event.fire('auth.logout', f);
        if (d || c) FB.Event.fire('auth.login', f);
        if (b) FB.Event.fire('auth.authResponseChange', f);
        if (h) FB.Event.fire('auth.statusChange', f);
        if (FB.Auth._refreshTimer) {
            window.clearTimeout(FB.Auth._refreshTimer);
            delete FB.Auth._refreshTimer;
        }
...
FB.provide('Canvas.EarlyFlush', {
    ...
    setCollectionMode: function (a) {
        ...
        if (a != FB.Canvas.EarlyFlush.COLLECT_AUTOMATIC && a != FB.Canvas.EarlyFlush.COLLECT_MANUAL) return false;
        FB.Canvas.EarlyFlush._collectionMode = a;
    },
    _maybeSample: function () {
        ...
        if (FB.Array.indexOf(FB.Canvas.EarlyFlush._appIdsBlacklist, parseInt(FB._apiKey, 10)) != -1) return;
        window.setTimeout(FB.Canvas.EarlyFlush._sample, 30000);
    },
    _sample: function () {
        var b = {
            object: 'data',
        ...
        if (FB.Canvas.EarlyFlush._collectionMode == FB.Canvas.EarlyFlush.COLLECT_AUTOMATIC) FB.Array.forEach(b, function (c, d) {
...
FB.provide('UIServer.MobileIframableMethod', {
    transform: function (a) {
        ...
        return FB.UIServer.genericTransform(a);
    },
    getXdRelation: function (a) {
        if (a === 'touch' && window.postMessage) return 'parent';
        ...
        if (element) {
            ...
            var processor = function () {
                ...
                if (tagInfo.className === 'FB.XFBML.LoginButton') {
                    ...
                    showFaces = addToProfile || getBoolAttr('show-faces') || getBoolAttr('show_faces');
                    isLogin = addToProfile || renderInIframe || showFaces || getBoolAttr('oneclick');
...
FB.subclass('XFBML.LoginButton', 'XFBML.ButtonElement', null, {
    ...
    getButtonMarkup: function () {
        ...
        case 'connected':
            if (FB.getAccessToken() && this._attr.autologoutlink) return FB.Intl._tx("Se d\u00e9connecter de Facebook");
            return this._getLoginText();
        default:
            FB.log('Unknown status: ' + this.status);
            return FB.Intl._tx("Connexion");
        }
    },
...
FB.subclass('XFBML.Read', 'XFBML.IframeWidget', null, {
    ...
    setupAndValidate: function () {
        ...
        function e(h) {
            if (h != 'manual' && h != 'onvisible') {
                var g = h.match(/^\d+(?:\.\d+)?%$/) ? parseInt(h, 10) : 80;
                g = Math.min(Math.max(g, 0), 100);
                h = g / 100;
            }
            return h;
        }
        this._attr = {
            ...
            href: this.getAttribute('href', window.location.href),
            ref: this.getAttribute('ref'),
            trigger: e(this.getAttribute('trigger', '')),
            mintime: parseInt(this.getAttribute('mintime'), 10) || 30
        };
        this._showLoader = false;
        this.subscribe('iframe.onload', FB.bind(function () {
            var g = this.dom.firstElementChild;
        ...
        var a = FB.bind(function () {
            ...
            window.setTimeout(FB.bind(this.arbiterInform, this, 'platform/plugins/read/action'), this._attr.mintime * 1000);
            return true;
        }, this);
        ...
        if (this._attr.trigger == "manual") {
            var c = FB.bind(function (g) {
                if (g == this._attr.href) {
                    FB.Event.unsubscribe('xfbml.read.read', c);
                    this.arbiterInform('platform/plugins/read/trigger');
                }
                return true;
            }, this);
            FB.Event.subscribe('xfbml.read.read', c);
        } else {
            var d = b(500, FB.bind(function () {
                if (this.calculateVisibility()) {
                    FB.Event.unlisten(window, 'scroll', d);
                    FB.Event.unlisten(window, 'resize', d);
                    this.arbiterInform('platform/plugins/read/trigger');
                }
                return true;
            }, this));
            FB.Event.listen(window, 'scroll', d);
            FB.Event.listen(window, 'resize', d);
            d();
        }
        this.visible = false;
        var f = b(500, FB.bind(function () {
            if (!this.visible && this.calculateVisibility()) {
                this.visible = true;
                this.arbiterInform('platform/plugins/read/visible');
            } else if (this.visible && !this.calculateVisibility()) {
                this.visible = false;
                this.arbiterInform('platform/plugins/read/invisible');
            }
            return true;
        }, this));
        FB.Event.listen(window, 'scroll', f);
        FB.Event.listen(window, 'resize', f);
        f();
        return true;
    },
    getSize: function () {
...
FB.XFBML.Read.markRead = function (a) {
    FB.Event.fire('xfbml.read.read', a || window.location.href);
};
...
FB.subclass('XFBML.ShareButton', 'XFBML.Element', null, {
    ...
    _renderButton: function (h) {
        ...
        case 'icon_link':
            ...
            break;
        case 'link':
            d = FB.Intl._tx("Partager sur Facebook");
            h = false;
            break;
        case 'button':
