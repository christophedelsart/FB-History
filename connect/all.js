...
Generated: October 27th 2010 4:13:47 AM PDT
Machine: 10.27.221.105
Location: JIT Construction: v306756
Locale: fr_FR
*/
...
if (!window.FB) FB = {
    ...
    _domain: {
        ...
        staticfbSsl: 'https://s-static.ak.facebook.com/',
        www: window.location.protocol + '//www.facebook.com/',
        wwwSsl: 'https://www.facebook.com/'
    },
    _locale: null,
    _localeIsRtl: false,
...
FB.provide('Content', {
    ...
    insertIframe: function (e) {
        ...
        if (document.attachEvent) {
            var b = ('<iframe' + ' id="' + e.id + '"' + ' name="' + e.name + '"' + (e.className ? ' class="' + e.className + '"' : '') + ' style="border:none;' + (e.width ? 'width:' + e.width + 'px;' : '') + (e.height ? 'height:' + e.height + 'px;' : '') + '"' + ' src="' + e.url + '"' + ' frameborder="0"' + ' scrolling="no"' + ' allowtransparency="true"' + ' onload="FB.Content._callbacks.' + a + '()"' + '></iframe>');
...
FB.provide('Flash', {
    ...
    init: function () {
        ...
        var a = !! document.attachEvent,
            c = FB._domain.cdn + FB.Flash._swfPath,
...
(function () {
    ...
    function quote(a) {
        ...
        return escapable.test(a) ? '"' + a.replace(escapable, function (b) {
            var c = meta[b];
    ...
    function str(d, a) {
        var b, c, h, e, f = gap,
            g, i = a[d];
        if (i && typeof i === 'object' && typeof i.toJSON === 'function') i = i.toJSON(d);
        if (typeof rep === 'function') i = rep.call(a, d, i);
        switch (typeof i) {
        case 'string':
            return quote(i);
        case 'number':
        ...
        case 'object':
            ...
            if (Object.prototype.toString.apply(i) === '[object Array]') {
                e = i.length;
                for (b = 0; b < e; b += 1) g[b] = str(b, i) || 'null';
                h = g.length === 0 ? '[]' : gap ? '[\n' + gap + g.join(',\n' + gap) + '\n' + f + ']' : '[' + g.join(',') + ']';
                gap = f;
                return h;
            }
            if (rep && typeof rep === 'object') {
                ...
                for (b = 0; b < e; b += 1) {
                    ...
                    if (typeof c === 'string') {
                        h = str(c, i);
                        if (h) g.push(quote(c) + (gap ? ': ' : ':') + h);
                    }
                }
            } else for (c in i) if (Object.hasOwnProperty.call(i, c)) {
                h = str(c, i);
                if (h) g.push(quote(c) + (gap ? ': ' : ':') + h);
            }
            h = g.length === 0 ? '{}' : gap ? '{\n' + gap + g.join(',\n' + gap) + '\n' + f + '}' : '{' + g.join(',') + '}';
    ...
    if (typeof JSON.stringify !== 'function') JSON.stringify = function (d, b, c) {
        var a;
        gap = '';
        indent = '';
        if (typeof c === 'number') {
            for (a = 0; a < c; a += 1) indent += ' ';
    ...
    if (typeof JSON.parse !== 'function') JSON.parse = function (text, reviver) {
        ...
        function walk(a, c) {
            var b, d, e = a[c];
            if (e && typeof e === 'object') for (b in e) if (Object.hasOwnProperty.call(e, b)) {
                d = walk(e, b);
                if (d !== undefined) {
                    e[b] = d;
                } else delete e[b];
...
FB.provide('ApiServer', {
    ...
    jsonp: function (b, f, d, e, a) {
        ...
        var h = (FB._domain[b] + f + (f.indexOf('?') > -1 ? '&' : '?') + FB.QS.encode(e));
...
FB.provide('Dialog', {
    ...
    _showLoader: function (a, c) {
        if (!FB.Dialog._loaderEl) {
            ...
            FB.Dialog._loaderEl = FB.Dialog._findRoot(FB.Dialog.create({
                content: ('<div class="dialog_title">' + '  <a id="fb_dialog_loader_close">' + '    <div class="fb_dialog_close_icon"></div>' + '  </a>' + '  <span>Facebook</span>' + '  <div style="clear:both;"></div>' + '</div>' + '<div class="dialog_content" style="width: ' + c + 'px;"></div>' + '<div class="dialog_footer"></div>')
            }));
        }
        if (!a) a = function () {};
    ...
    create: function (e) {
        ...
        if (FB.Dom.getBrowserType() == 'ie') {
            a += ' fb_dialog_legacy';
            FB.Array.forEach(['vert_left', 'vert_right', 'horiz_top', 'horiz_bottom', 'top_left', 'top_right', 'bottom_left', 'bottom_right'], function (f) {
...
FB.provide('XD', {
    ...
    handler: function (a, e, b) {
        ...
        var f = FB._domain.cdn + 'connect/xd_proxy.php#',
            c = FB.guid();
...
FB.provide('Arbiter', {
    inform: function (b, d, e, g) {
        ...
        var h = (FB._domain[g ? 'staticfbSsl' : 'staticfb'] + 'connect/canvas_proxy.php#' + FB.QS.encode({
            method: b,
...
FB.provide('UIServer', {
    ...
    prepareCall: function (h, b) {
        var g = h.method.toLowerCase(),
            f = FB.UIServer.Methods[g],
            e = FB.guid(),
            d = (g !== 'auth.status');
        if (!f) {
            FB.log('"' + g + '" is an unknown method.');
            return;
        }
        FB.copy(h, {
            api_key: FB._apiKey,
            locale: FB._locale,
            sdk: 'joey',
            access_token: d && FB._session && FB._session.access_token
        });
        h.display = FB.UIServer.getDisplayMode(f, h);
        if (!f.url) f.url = 'connect/uiserver.php';
        var a = {
            cb: b,
            id: e,
            size: f.size || {},
            url: FB._domain[d ? 'wwwSsl' : 'www'] + f.url,
            params: h
        };
...
FB.subclass('XFBML.ConnectBar', 'XFBML.Element', null, {
    ...
    _renderConnectBar: function (d) {
        ...
        b.innerHTML = FB.String.format('<div class="fb_buttons">' + '<a href="#" class="fb_bar_close">' + '<img src="{1}" alt="{2}" title="{2}"/>' + '</a>' + '</div>' + '<a href="{7}" class="fb_profile" target="_blank">' + '<img src="{3}" alt="{4}" title="{4}"/>' + '</a>' + '{5}' + ' <span>' + '<a href="{8}" class="fb_learn_more" target="_blank">{6}</a> &ndash; ' + '<a href="#" class="fb_no_thanks">{0}</a>' + '</span>', FB.Intl._tx("Non merci"), FB._domain.cdn + FB.XFBML.ConnectBar.imgs.buttonUrl, FB.Intl._tx("Fermer"), d[this._picFieldName] || FB._domain.cdn + FB.XFBML.ConnectBar.imgs.missingProfileUrl, FB.String.escapeHTML(d.first_name), FB.Intl._tx("Bonjour {firstName}. <strong>{siteName}<\/strong> utilise Facebook pour personnaliser votre navigation.", {
            firstName: FB.String.escapeHTML(d.first_name),
            siteName: FB.String.escapeHTML(d.site_name)
        }), FB.Intl._tx("En savoir plus"), d.profile_url, FB._domain.www + 'sitetour/connect.php');
        var a = this;
...
FB.subclass('XFBML.LikeBox', 'XFBML.IframeWidget', null, {
    ...
    setupAndValidate: function () {
        this._attr = {
            ...
            stream: this._getBoolAttribute('stream', true),
            width: this._getPxAttribute('width', 300),
            show_faces: this._getBoolAttribute('show-faces', false),
        ...
        if (!a) if ((!this._attr.connections || this._attr.connections === '0') && !this._attr.stream) {
            ...
            a = 95;
            if (this._attr.connections && this._attr.connections !== '0') a += 160;
            if (this._attr.stream) a += 300;
            if (this._attr.header && this._attr.header !== '0' && !this._attr.href) a += 32;
        }
        this._attr.height = a;
...
FB.subclass('XFBML.Name', 'XFBML.Element', null, {
    ...
    _renderOther: function (c) {
        ...
        if (this._uid == FB.Helper.getLoggedInUser() && this._getBoolAttribute('use-you', true)) {
            ...
            } else if (this._lastnameonly) b = FB.String.escapeHTML(c.last_name);
...
FB.subclass('XFBML.ProfilePic', 'XFBML.Element', null, {
    process: function () {
        ...
        var c = this.bind(function (j) {
            var l = j ? j[0] : null,
                i = l ? l[b] : null;
            if (!i) i = FB._domain.cdn + FB.XFBML.ProfilePic._defPicMap[b];
...
FB.subclass('XFBML.Send', 'XFBML.EdgeWidget', null, {
    ...
    _createEdgeCommentWidget: function (b, a) {
        var c = {
            ...
            height: 300,
            relativeHeightOffset: this._getHeightOffset(),
            relativeWidthOffset: this._getWidthOffset()
        };
...
FB.subclass('XFBML.ShareButton', 'XFBML.Element', null, {
    ...
    _renderButton: function (f) {
        ...
        var b = '',
            ...
            d = '',
            a = '',
            e = FB.Intl._tx("Partager"),
            g = '';
        switch (this._type) {
        case 'icon':
        case 'icon_link':
            a = 'fb_button_simple';
            b = ('<span class="fb_button_text">' + (this._type == 'icon_link' ? e : '&nbsp;') + '</span>');
            f = false;
            break;
        case 'link':
            b = FB.Intl._tx("Partager sur Facebook");
            f = false;
            break;
        case 'button':
        ...
        this.dom.innerHTML = FB.String.format('<span class="{0}">{4}<a href="{1}" class="{2}" ' + 'onclick=\'FB.ui({6});return false;\'' + 'target="_blank">{3}</a>{5}</span>', g, this._href, a, b, d, c, FB.JSON.stringify({
            method: 'stream.share',
            u: this._href
        }));
